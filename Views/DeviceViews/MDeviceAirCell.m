...
-(IBAction)airRemoteAction:(id)sender
{
    ...
    NSString *sendMsg = [NSString stringWithFormat:@"%@&%d&%d&%d&%d&%d&%d&%d&%d&%d",_entityRemote.entityId,_entityRemote.brandType,_entityRemote.remoteBrandIndex,_entityRemote.remoteGroupIndex,airKeyValue,switchStatus,modeStatus,celsiusStatusTen * 10 + celsiusStatusBits,windStatus,_sweptStatus];
    
    NSLog(@"空调数据发送:%@",sendMsg);
    
    [Tool soundAction];
    [[Interface shareInterface:nil] writeFormatDataAction:@"9" didMsg:sendMsg didCallBack:^(NSString *code){
        NSLog(@"msg:%@",code);
    }];
