...
-(void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if (style == Verify) {
        if ([rerificationDelegate respondsToSelector:@selector(verification:)]) {
            success = [rerificationDelegate verification:resultString];
        }
    }else{
        if ([resetDelegate respondsToSelector:@selector(resetPassword:)]) {
...
- (void)drawRect:(CGRect)rect {
    ...
    for (int i= 0 ; i<touchesArray.count; i++) {
        ...
        if (success) {
            CGContextSetRGBStrokeColor(context, 2/244.f, 174/255.f, 240/255.f, 0.7); //线条颜色
        }else {
